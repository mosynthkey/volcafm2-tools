<template>
  <svg class="algorithm-diagram" viewBox="0 0 240 210" role="img" :aria-label="`Algorithm ${algorithm + 1}`">
    <defs>
    ...
    <path v-for="node in carriers" :key="`out${node.id}`" class="output-line" :d="`M${node.x} ${node.y + 10}V178`" />
...
const outputBusPath = computed(() => {
  ...
  return `M${left} 178H${right}M120 178V198`;
});
...
const edgePath = (fromId: number, toId: number) => {
  ...
  return `M${from.x} ${from.y + 10}V${middleY}H${to.x}V${to.y - 13}`;
};
...
<style scoped>
.algorithm-diagram { width: 100%; height: 250px; border: 1px solid rgba(241,233,225,.18); border-radius: 8px; background: #1c1516; }
.routing-line { fill: none; stroke: #f1e9e1; stroke-width: 1.9; stroke-linecap: round; stroke-linejoin: round; }
.feedback-line { fill: none; stroke: #72d5ca; stroke-width: 2.2; stroke-linecap: round; stroke-dasharray: 3 2; }
