<template>
  <div class="operator-overview">
    <article v-for="(operator, index) in program.operators" :key="index" class="operator-summary" :class="{ off: !operator.enabled }">
      <div class="operator-summary-heading">
        <button type="button" class="operator-select" :aria-label="t('sound.operator', { count: index + 1 })"
        ...
        <div class="mode-segment" role="group" :aria-label="t('sound.oscillatorMode')">
          <button type="button" :class="{ active: operator.oscillatorMode === 0 }"
            :aria-pressed="operator.oscillatorMode === 0" @click="operator.oscillatorMode = 0">{{ t('sound.ratio') }}</button>
          <button type="button" :class="{ active: operator.oscillatorMode === 1 }"
            :aria-pressed="operator.oscillatorMode === 1" @click="operator.oscillatorMode = 1">{{ t('sound.fixed') }}</button>
        </div>
        <button type="button" class="operator-enable" :class="{ enabled: operator.enabled }"
          :aria-pressed="operator.enabled" :aria-label="t('sound.operator', { count: index + 1 })"
          @click="operator.enabled = !operator.enabled">
          {{ operator.enabled ? t('common.on') : t('common.off') }}
        </button>
      </div>
      ...
      <div class="mini-graphs">
        <div>
          ...
          <EnvelopeCopyMenu compact :rates="operator.egRates" :levels="operator.egLevels"
            @apply="(rates, levels) => applyEnvelope(index, rates, levels)">
      ...
      <dl class="parameter-grid">
        <template v-for="item in frequencyParameters(operator)" :key="item.label">
          <div><dt>{{ item.label }}</dt><dd>
            <button type="button" class="drag-value" :aria-label="`${item.label} ${item.value}`"
      ...
      <dl class="scaling-row">
        <template v-for="item in scalingParameters(operator)" :key="item.label">
          <div><dt>{{ item.label }}</dt><dd>
            <button v-if="item.curve !== undefined && item.field" type="button" class="curve-value"
              :aria-label="`${item.label} ${item.value}`" @click="cycleCurve(index, item.field)">
              <MiniCurve :curve="item.curve" :direction="item.direction" />
            </button>
            <button v-else-if="item.field" type="button" class="drag-value" :aria-label="`${item.label} ${item.value}`"
              @pointerdown="startDrag($event, index, item.field, item.min, item.max, item.rawValue)"
              @pointermove="moveDrag" @pointerup="endDrag" @pointercancel="endDrag"
              @keydown="adjustWithKeyboard($event, index, item.field, item.min, item.max, item.rawValue)">
              <span>{{ item.value }}</span><MiniKnob :value="item.rawValue" :min="item.min" :max="item.max" />
...
const cycleCurve = (operatorIndex: number, field: 'leftCurve' | 'rightCurve') => {
  setValue(operatorIndex, field, 0, 3, (program.value.operators[operatorIndex][field] + 1) % 4);
};

const frequencyLabel = formatOperatorFrequency;
const envelopePoints = (operator: SoundOperator) => dx7EnvelopePoints(operator.egRates, operator.egLevels, 180, 62, 6);
const applyEnvelope = (operatorIndex: number, rates: number[], levels: number[]) => {
  const operator = program.value.operators[operatorIndex];
  operator.egRates = [...rates];
  operator.egLevels = [...levels];
};
...
const frequencyParameters = (operator: SoundOperator) => [
...
const scalingParameters = (operator: SoundOperator) => [
  { label: t('sound.compact.leftCurve'), value: curveNames[operator.leftCurve], curve: operator.leftCurve, direction: 'right' as const, field: 'leftCurve' as const },
  ...
  { label: t('sound.compact.rightCurve'), value: curveNames[operator.rightCurve], curve: operator.rightCurve, direction: 'left' as const, field: 'rightCurve' as const },
];
</script>
...
.operator-select:hover { background: rgba(206,179,147,.08); }
.operator-select:focus-visible, .operator-enable:focus-visible, .mode-segment button:focus-visible, .curve-value:focus-visible { outline: 2px solid #e1cab0; outline-offset: -2px; }
.operator-select strong { width: 27px; height: 27px; display: grid; place-items: center; border-radius: 6px; background: rgba(206,179,147,.14); color: #e1cab0; font-size: var(--volca-type-body); }
.operator-select span { overflow: hidden; font-size: var(--volca-type-label); font-weight: 700; text-overflow: ellipsis; white-space: nowrap; }
.mode-segment { height: 26px; display: grid; grid-template-columns: 1fr 1fr; padding: 2px; box-sizing: border-box; border: 1px solid rgba(206,179,147,.3); border-radius: 6px; background: #251c1e; }
.mode-segment button { width: 46px; height: 22px; padding: 0 6px; border: 0; background: transparent; color: #ad9e96; font: inherit; font-size: 10px; font-weight: 700; cursor: pointer; }
...
.parameter-grid dt, .scaling-row dt { overflow: hidden; color: #9f918a; font-size: 10px; line-height: 1.2; text-overflow: ellipsis; white-space: nowrap; }
.parameter-grid dd, .scaling-row dd { min-height: 30px; margin: 1px 0 0; color: #e1cab0; font-size: 12px; font-weight: 750; font-variant-numeric: tabular-nums; }
.scaling-row { display: grid; grid-template-columns: repeat(6, minmax(0, 1fr)); gap: 1px; margin: 0 7px 6px; overflow: hidden; border-radius: 5px; background: rgba(206,179,147,.1); }
.scaling-row .drag-value { min-width: 0; width: 100%; grid-template-columns: minmax(12px, 1fr) 30px; padding: 1px 2px; }
.scaling-row .mini-curve { width: 100%; max-width: 38px; }
.drag-value { min-width: 56px; display: inline-grid; grid-template-columns: minmax(18px, auto) 30px; align-items: center; justify-content: end; gap: 3px; padding: 1px 3px 1px 4px; border: 0; border-radius: 4px; background: rgba(206,179,147,.08); color: #e1cab0; font: inherit; font-weight: 750; cursor: ns-resize; touch-action: none; }
...
.mini-knob { width: 30px; height: 30px; overflow: visible; }
...
.eg-values span, .eg-values > button { padding: 3px 4px; background: #271d1f; font-size: 11px; line-height: 1.25; }.eg-values span { color: #9f918a; }.eg-values > button { width: 100%; grid-template-columns: minmax(14px, auto) 30px; gap: 2px; border-radius: 0; color: #d8ccc4; text-align: center; font-variant-numeric: tabular-nums; }
@media (max-width: 1180px) { .operator-overview { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
</style>
