<template>
  <div class="volca-inline-panel motion-control" :class="{ collapsed }" @click="collapsed && (collapsed = false)">
    ...
    <div class="volca-groups">
      <section class="volca-group">
        ...
        <div v-show="!collapsed" class="control-row">
      ...
      <section class="volca-group auto-write-group">
        ...
        <div v-show="!collapsed" class="pattern-panel" :aria-label="t('sequence.autoMotion')">
          <div class="pattern-settings">
            ...
            <label><span>{{ t('sequence.motionOffset') }}</span><select v-model.number="patternOffset"><option v-for="offset in 16" :key="offset - 1" :value="offset - 1">{{ offset - 1 }}</option></select></label>
          </div>
...
const collapsed = usePersistedFlag('volca-fm2-collapse-motion-panel')
...
const patternRange = () => {
  ...
  return { min, max, cycles: patternCycles.value, offset: patternOffset.value }
}
const patternPoints = (key: MotionPatternKey) => {
  const { min, max, cycles, offset } = patternRange()
  return previewMotionPattern(key, min, max, cycles, offset).map((value, index) => `${index * (160 / 15)},${55 - value / 127 * 52}`).join(' ')
}
const squarePatternPath = () => {
  const { min, max, cycles, offset } = patternRange()
  const values = previewMotionPattern('square', min, max, cycles, offset)
