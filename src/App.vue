...
const NAV_TEXTS = {
  ja: { dx7: '音色をDX7 SysExに変換', sequencer: 'シーケンスを編集' },
  en: { dx7: 'Receive & convert patches', sequencer: 'Edit the 16-step pattern' },
};
...
.brand-mark { width: 38px; height: 38px; display: grid; place-items: center; position: relative; border: 1px solid var(--volca-line-strong); border-radius: 10px; background: linear-gradient(145deg, #e0c5a5, #a8896c); color: #332729; box-shadow: inset 0 1px rgba(255,255,255,.35), 0 5px 18px rgba(0,0,0,.22); font-size: 12px; font-weight: 900; letter-spacing: -.05em; }
...
.nav-item small { overflow: hidden; color: #aa9b9b; font-size: 11px; line-height: 1.35; text-overflow: ellipsis; white-space: nowrap; }
.sidebar-fill { flex: 1; }
.sidebar-footer { display: grid; gap: 5px; margin: 4px; padding-top: 12px; border-top: 1px solid var(--volca-line); }
.about-button { display: flex; align-items: center; gap: 9px; min-height: 38px; padding: 7px 10px; border: 0; border-radius: 9px; background: transparent; color: var(--volca-muted); font: inherit; font-size: 13px; font-weight: 600; text-align: left; cursor: pointer; transition: background .16s ease, color .16s ease, transform .1s ease; }
...
.workspace .v-btn { border: 1px solid rgba(255,255,255,.08); border-radius: 8px; background: var(--volca-accent) !important; color: #33282a !important; box-shadow: inset 0 1px rgba(255,255,255,.28), 0 3px 10px rgba(0,0,0,.16) !important; font-size: 13px; font-weight: 700; letter-spacing: .005em; text-transform: none; }
.workspace .v-btn:hover { background: var(--volca-accent-bright) !important; }
.workspace .v-btn:active { transform: scale(.98); }
.workspace .v-field { border-radius: 8px !important; background: rgba(20,15,16,.38); font-size: 14px; }
.workspace .v-label { font-size: 13px; }
.workspace .v-divider { border-color: var(--volca-line); opacity: 1; }
.workspace .v-card-title { font-size: 17px; font-weight: 650; letter-spacing: -.01em; }
...
.connection-card .v-card-title { padding-bottom: 8px; font-size: 19px; font-weight: 680; letter-spacing: -.02em; }
.connection-card .v-card-text { color: #d8ccc4; font-size: 15px; line-height: 1.75; }
