...
export const previewMotionPattern = (
  key: MotionPatternKey,
  min: number,
  max: number,
  cycles: number,
  ...
  let randomIndex = 0;
  return key === 'random'
    ? createMotionPattern(key, { min, max, cycles, offset, random: () => previewRandom[randomIndex++] / 127 })
    : createMotionPattern(key, { min, max, cycles, offset });
};
